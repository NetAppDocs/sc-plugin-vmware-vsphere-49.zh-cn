...
.步骤
. 在选择控制器插件的左侧导航器窗格中、单击*资源组*、选择一个资源组、然后单击 image:scpivs44_image39.png["编辑图标"] * 编辑资源组 * 以启动向导。
. 在 * 资源 * 页面上，执行以下操作：
+
...
+
单击 * 下一步 * 时，系统会首先检查 SnapCenter 是否管理选定虚拟机或数据存储库所在的存储并与其兼容。如果消息 `Some entities are not SnapCenter compatible` 此时将显示、选定的虚拟机或数据存储库与SnapCenter 不兼容。请参见 link:scpivs44_create_resource_groups_for_vms_and_datastores.html#manage-compatibility-check-failures["管理兼容性检查失败"] 有关详细信息 ...
